...
<template>
  ...
  >
    ...
    >
      ...
      <!-- 商品信息 -->
      <div
        class="flex-1 pl5 pr5"
        :style="getStyle('item-info')"
      >
        <div>
          <div
          ...
          >{{item.name}}</div>
          <div
            v-show="attr.showInfo"
            class="mt8 f12 f-grey"
          >{{item.describe}}</div>
        </div>
        ...
        >
          <div class="mb5">
            ...
            <!-- 原价 -->
            <span
              v-show="attr.showOriginalPrice"
              class="text-through f-grey"
            >
              <span class="ml4 f12">¥</span>
              <span
                class="f14"
                :style="getStyle('originalPrice')"
              >{{item.originalPrice | money}}</span>
            </span>
          </div>

          <!-- 购买按钮 -->
          <div
            class="flex-center mb5 f-white"
            :style="getStyle('buy')"
          >
            <i
  ...
  methods: {
    getStyle(target) {
      ...
      if (target == "image") {
        if (this.attr.listModel == "oneCols") {
          if (this.attr.model == "vertical") {
            return { width: this.$pxTorem(108), marginRight: this.$pxTorem(5) };
          } else {
            return {
              width: this.$pxTorem(140),
      ...
      if (target == "item-info") {
        if (this.attr.listModel == "oneCols") {
          return {
            ...
            paddingLeft: this.$pxTorem(8),
            paddingRight: this.$pxTorem(8),
          };
        } else {
      ...
      // 价格块样式
      if (target == "priceCtn") {
        if (this.attr.listModel != "oneCols" && this.attr.itemAlign == "center") {
          return {
      ...
      if (target == "buy") {
        ...
        if (this.attr.buyText) {
          result.borderRadius = this.$pxTorem(4);
          result.padding = `${this.$pxTorem(4)} ${this.$pxTorem(8)}`;
        } else {
          result.borderRadius = `50%`;
          result.width = this.$pxTorem(24);
          result.height = this.$pxTorem(24);
        }
      ...
      if (
        ...
        if (target == "price") {
          return {
            fontSize: `${this.$pxTorem(15)} !important`,
          };
        }
        if (target == "originalPrice") {
          return {
            fontSize: `${this.$pxTorem(12)} !important`,
          };
        }
        if (target == "buy-icon") {
          return {
            fontSize: `${this.$pxTorem(16)} !important`,
          };
        }
      }
    },
